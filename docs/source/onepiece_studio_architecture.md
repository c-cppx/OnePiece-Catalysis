...
### OnePiece Studio tests

Place under:

```text
tests/onepiece_studio/
```
...
- browser interaction
- project UI behavior
- rendering and table/plot selection

## Immediate Success Criteria

The migration is working when:

1. OnePiece Studio no longer contains the main DataFrame operation dispatcher
2. Controlroom filtering is executed by OnePiece
3. project save/load can be interpreted without Streamlit-specific knowledge
4. scientific tests run against OnePiece directly
5. OnePiece Studio tests can be thinner and focus on user interaction

## Recommended First Public Framing

When the refactor is ready, describe the software like this:

> `onepiece-studio` is a local scientific database workbench for ASE/pandas
> datasets. `onepiece` provides the backend workflow and analysis engine; `onepiece_studio`
> provides the interactive desktop-style interface for browsing, filtering,
> curating, visualizing, and reproducing materials-science analyses.
